...
// registro más sencillo, donde está en la misma funcón todo el código
//! -----------------------------------------------------------------------------
//? ----------------------------REGISTER LARGO EN CODIGO ------------------------
//! -----------------------------------------------------------------------------
//------------------->CRUD es el acrónimo de "Crear, Leer, Actualizar y Borrar". // estamos haciendo la parte del create
// función asincrona porque el backend es asíncrono
const registerLargo = async (req, res, next) => {
  // lo primero que vamos hacer es captura la imagen que previamente hemos subido en el middleaware
  ...
  try {
    //? ---> hay que meter un try catch por cada asincronia de actualizacion que tengamos de actualizacion para poder seleccionar los errores de forma separada e individualizada
    ...
    const userExist = await User.findOne(
      // primero el modelo, quiero que me busques uno que coincida o bien en el mail o bien en el correo
    ...
    if (!userExist) {
      ...
      try {
        ...
        return res.status(404).json(error.message); // es dinámico, por eso no le damos un mensaje
      }
    } else {
...
module.exports = { registerLargo }; // exportamos el resgistro
